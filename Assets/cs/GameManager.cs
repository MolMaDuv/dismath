...
public class GameManager : MonoBehaviour
{
    ...
    public Tabla[] listaTablas = new Tabla[3];
    ...
    //Vector3 escala = new Vector3(1.990355f,1.990355f,20f);
    
    void Start()
    {
        float n = Random.Range(1,12);
        int indice = Random.Range(0,3);
       
        print(indice);
        listaTablas[3].modificarEscala(n);
        listaTablas[indice].modificarEscala(n);


        for(int i = 0; i<=2; i++){

            if(i!=indice){
                float x = Random.Range(1,12);
                listaTablas[i].modificarEscala(x);
            }
        }
    }
    ...
    {
        texto.text = "incorrecto esta tabla no es del mismo tama�o que la de muestra, toca el boton de nuevo para devolvier la ficha al su lugar ";
    ...
    public void pressboton3()
    {
        ...
        texto.text = "excelente, las tablas  son del mismo tama�o, presiona el boton siguiente";
